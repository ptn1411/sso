...
export function JwtSignAccessToken(payload: string | Buffer | object, exp: number): Promise<FunSign> {
  try {
...
export function JwtVerifyAccessToken(token: string): Promise<FunExperienceResult> {
  const secretAccess = process.env.ACCESS_TOKEN_PRIVATE_KEY as string
...
export function JwtSignRefreshToken(payload: string | Buffer | object, exp: number): Promise<FunSign> {
...
export function JwtVerifyRefreshToken(token: string): Promise<FunExperienceResult> {
  try {
    const secretRefresh = process.env.REFRESH_TOKEN_PRIVATE_KEY as string
...
export async function JwtGenerateTokens(payload: string | Buffer | object): Promise<FunGenerateTokens> {
  try {
