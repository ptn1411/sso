import React, { ChangeEvent, FC, ReactNode, useContext, useId, useState, useEffect } from 'react'
...
const Register: FC<Props> = ({ children, ...props }) => {
  ...
  const error = useSelector((state: ReduxState) => state.app.register.error)
  const user = useSelector((state: ReduxState) => state.app.register.user)
  ...
  const onClickRegister = async (e: HTMLFormElement) => {
    ...
    try {
      ...
      dispatch({ type: REGISTER_USER.REQUEST, payload: { user } })
    } catch (e: any) {
  ...
  return (
    <>
      ...
      <form className={classes.form}>
        ...
        <div>
          <label className={classes.label} htmlFor={nameId}>
            Name
          </label>
          <input
        ...
        <div>
          ...
          <input
            ...
            id={emailId}
          />
          {error?.name === 'email' ? <div className={classes.errorMsg}>{error?.message}</div> : null}
        </div>
        <div>
          ...
          <input
            ...
            id={usernameId}
          />
          {error?.name === 'username' ? <div className={classes.errorMsg}>{error?.message}</div> : null}
        </div>
        <div className={classes.divBlock}>
          ...
          <input
            ...
            id={passwordId}
          />
          <span onClick={handleClickShowPassword} className={classes.span}>
        ...
        <button onClick={onClickRegister} className={classes.button}>
          Register
        </button>
      </form>
    </>
